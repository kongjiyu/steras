...
import { sterasTestAdminEnvironment, sterasTestAdminScriptPath } from './global-setup';
...
async function run(action: string, argument?: string): Promise<string> {
  const { stdout } = await execFileAsync(
    process.execPath,
    [sterasTestAdminScriptPath(), action, ...(argument ? [argument] : [])],
    { env: sterasTestAdminEnvironment(), maxBuffer: 10 * 1024 * 1024 },
  );
  return stdout.trim();
}
