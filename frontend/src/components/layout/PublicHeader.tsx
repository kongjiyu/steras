    ...
    <header className="sticky top-0 z-40 border-b border-[#e4dac7] bg-[#fffdf7]/95 backdrop-blur-sm">
      <div className="mx-auto flex min-h-[72px] max-w-6xl items-center justify-between gap-3 px-5 sm:px-8">
        ...
        <nav className="flex items-center gap-1 sm:gap-3" aria-label="Public navigation">
          ...
          <NavLink to={getIncidentPath(profile?.role)} end={profile?.role === 'public'} className={({ isActive }) => `${profile?.role === 'public' ? 'inline-flex' : 'hidden sm:inline-flex'} min-h-11 items-center rounded-md px-3 py-2 text-sm font-semibold ${isActive ? 'bg-[#edf2dc] text-[#52651c]' : 'text-[#5d5b4e] hover:bg-[#f7f1e5]'}`}>{profile?.role === 'public' ? 'My reports' : 'Report incident'}</NavLink>
