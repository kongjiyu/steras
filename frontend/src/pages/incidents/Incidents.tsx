...
function StaffIncidentQueue({ incidents, selected, loading, loadError, organizer, authority, queueFilter, counts, onQueueFilterChange, onSelect }: {
  ...
  return <section className="card h-fit overflow-hidden">
    ...
    <div className="max-h-[60vh] overflow-y-auto divide-y divide-[#eee8dc]">
      {incidents.map((item) => <button key={item.incidentId} type="button" aria-label={`${item.eventName} incident`} onClick={() => onSelect(item.incidentId)} className={`block w-full p-4 text-left transition ${selected === item.incidentId ? 'bg-brand-50' : 'hover:bg-cream-50'}`}>
        <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-semibold text-ink-800">{selected === item.incidentId ? 'Selected incident' : item.eventName}</p><p className="mt-1 text-xs text-ink-500">{INCIDENT_CATEGORY_LABELS[item.category]}</p></div><div className="flex shrink-0 flex-wrap justify-end gap-1.5">{!organizer && !authority && <Status value={item.status} />}{item.severity && <SeverityBadge value={item.severity} />}</div></div>
