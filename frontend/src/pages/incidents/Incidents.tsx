...
function IncidentDetail({ record, profile, directory, busy, setBusy, onDone, setError }: { record: IncidentView; profile: NonNullable<ReturnType<typeof useAuth>['profile']>; directory: M4AuthorityDirectoryEntry[]; busy: boolean; setBusy: (v: boolean) => void; onDone: () => Promise<void>; setError: (v: string) => void }) {
  ...
  return <section className="space-y-4"><article className="card"><div className="card-header"><div><h2 className="section-title">{record.eventName}</h2><p className="text-xs text-ink-500">{record.incidentId}</p></div><Status value={record.status} /></div><div className="card-body space-y-4 text-sm">{record.activityClosed && <div className="rounded-md bg-warning-50 p-3 text-warning-900"><strong>Activity closed</strong><p className="mt-1">This incident is retained for history, but further action is disabled because the event was withdrawn.</p></div>}<p>{record.description}</p><dl className="grid gap-3 sm:grid-cols-2"><div><dt className="field-label">Location</dt><dd>{record.location}</dd></div><div><dt className="field-label">Severity</dt><dd>{record.severity ?? 'Manual review required'}</dd></div><div><dt className="field-label">AI assessment</dt><dd>{record.aiAssessment.status === 'success' ? `${record.aiAssessment.rationale} · ${record.aiAssessment.immediateActionRequired ? 'Immediate action required' : 'No immediate action indicated'}` : `Unavailable: ${record.aiAssessment.reason}`}</dd></div><div><dt className="field-label">Evidence</dt><dd>{record.evidence.length ? `${record.evidence.length} verified upload(s)` : 'None supplied'}</dd></div></dl><EvidenceLinks incidentId={record.incidentId} evidence={record.evidence} setError={setError} />{record.finalResolution && <div className="rounded-md bg-brand-50 p-3"><strong>Final resolution</strong><p className="mt-1">{record.finalResolution}</p></div>}</div></article>
