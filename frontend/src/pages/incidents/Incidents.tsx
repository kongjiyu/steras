...
export default function Incidents() {
  ...
  return <>
    ...
    <main className={`${civicWorkspace ? 'page-shell page-enter' : standalonePublic || organizerListPage ? 'min-h-screen bg-cream-50 px-5 py-8 sm:px-8' : ''} ${standalonePublic && !submissionPage ? 'incident-history-compact' : ''}`}>
      ...
      {profile?.role === 'organizer' && !organizerListPage && <OrganizerSummary actionRequired={organizerCounts.actionRequired} highSeverity={organizerCounts.highSeverity} resolutionReview={organizerCounts.resolutionReview} />}
...
function StaffIncidentQueue({ incidents, selected, loading, loadError, organizer, authority, queueFilter, counts, onQueueFilterChange, onSelect }: {
  ...
  return <section className="card h-fit overflow-hidden">
    <div className="card-header"><div><h2 className="section-title">Incident queue</h2>{!organizer && <p className="text-xs text-ink-500">{loading ? 'Loading records...' : loadError ? 'Record count unavailable' : `${incidents.length} accessible records`}</p>}</div><Activity size={18} /></div>
    ...
    <div className="max-h-[60vh] overflow-y-auto divide-y divide-[#eee8dc]">
      {incidents.map((item) => <button key={item.incidentId} type="button" onClick={() => onSelect(item.incidentId)} className={`block w-full p-4 text-left transition ${selected === item.incidentId ? 'bg-brand-50' : 'hover:bg-cream-50'}`}>
...
function OrganizerIncidentList({ incidents, total, page, totalPages, setPage, counts, search, category, filter, setSearch, setCategory, setFilter, loading, loadError }: {
  ...
  const tabs: Array<[OrganizerListFilter, string, number]> = [
    ['all', 'All records', counts.all], ['ongoing', 'Ongoing', counts.ongoing], ['pending', 'Pending action', counts.pending], ['closed', 'Closed', counts.closed],
  ];
    ...
    <div className="card-body space-y-4">
      {canChooseResponsePath && <>
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <button type="button" aria-expanded={openPath === 'authority'} className={`rounded-md border p-4 text-left transition ${openPath === 'authority' ? 'border-ink-900 bg-cream-50 ring-1 ring-ink-900' : 'border-[#e4ddcf] hover:border-brand-400 hover:bg-cream-50'}`} onClick={() => setOpenPath('authority')}>
...
function organizerStatusLabel(record: IncidentView) {
  ...
  if (ORGANIZER_ASSIGNMENT_STATUSES.has(record.status)) return 'Action Required';
