...
function IncidentDetail({ record, profile, directory, busy, setBusy, onDone, setError }: { record: IncidentView; profile: NonNullable<ReturnType<typeof useAuth>['profile']>; directory: M4AuthorityDirectoryEntry[]; busy: boolean; setBusy: (v: boolean) => void; onDone: () => Promise<void>; setError: (v: string) => void }) {
  ...
  const act = async (action: string, extra: Record<string, unknown> = {}) => { setBusy(true); setError(''); try { const paths = await uploadEvidence(profile.uid, actionFiles, crypto.randomUUID()); const fn = httpsCallable(functions, 'manageIncident'); await fn({ incidentId: record.incidentId, action, note, evidencePaths: paths, ...extra }); setNote(''); setActionFiles([]); await onDone(); } catch (e) { setError(e instanceof Error ? e.message : 'Action failed.'); } finally { setBusy(false); } };
  return <section className="space-y-4"><article className="card"><div className="card-header"><div><h2 className="section-title">{record.eventName}</h2><p className="text-xs text-ink-500">{record.incidentId}</p></div><Status value={record.status} /></div><div className="card-body space-y-4 text-sm"><p>{record.description}</p><dl className="grid gap-3 sm:grid-cols-2"><div><dt className="field-label">Location</dt><dd>{record.location}</dd></div><div><dt className="field-label">Severity</dt><dd>{record.severity ?? 'Manual review required'}</dd></div><div><dt className="field-label">AI assessment</dt><dd>{record.aiAssessment.status === 'success' ? `${record.aiAssessment.rationale} · ${record.aiAssessment.immediateActionRequired ? 'Immediate action required' : 'No immediate action indicated'}` : `Unavailable: ${record.aiAssessment.reason}`}</dd></div><div><dt className="field-label">Evidence</dt><dd>{record.evidence.length ? `${record.evidence.length} verified upload(s)` : 'None supplied'}</dd></div></dl><EvidenceLinks incidentId={record.incidentId} evidence={record.evidence} setError={setError} />{record.finalResolution && <div className="rounded-md bg-brand-50 p-3"><strong>Final resolution</strong><p className="mt-1">{record.finalResolution}</p></div>}</div></article>
...
function EvidenceLinks({ incidentId, evidence, setError }: { incidentId: string; evidence: M4IncidentRecord['evidence']; setError: (message: string) => void }) {
  if (!evidence.length) return null;
  const open = async (path: string) => {
    try {
      const callable = httpsCallable<{ incidentId: string; path: string }, { url: string }>(functions, 'getIncidentEvidenceDownloadUrl');
      ...
      const anchor = document.createElement('a'); anchor.href = result.data.url; anchor.target = '_blank'; anchor.rel = 'noopener noreferrer'; anchor.click();
    } catch (error) { setError(error instanceof Error ? error.message : 'Evidence could not be opened.'); }
  };
  return <div className="mt-2 flex flex-wrap gap-2">{evidence.map((item) => <button type="button" className="btn-secondary text-xs" key={item.path} onClick={() => void open(item.path)}>{item.name}</button>)}</div>;
}
