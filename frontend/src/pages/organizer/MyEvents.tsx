  ...
  return (
    <div>
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto border-b border-[#ded5c5] pb-3" aria-label="Filter applications by status">
        {(['all', 'Draft', 'Pending', 'UnderReview', 'Approved', 'Rejected', 'Withdrawn', 'Manual Review Required'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
              ...
              {filtered.map((e) => (
                <tr key={e.eventId} className="transition-colors hover:bg-cream-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link to={e.status === 'Draft' ? `/organizer/events/${e.eventId}/edit` : `/organizer/events/${e.eventId}`} className="text-brand-600 hover:text-brand-700 text-sm font-medium">
