...
export default function NewEvent() {
  ...
  const [editableStatus, setEditableStatus] = useState<'Draft' | 'AmendmentRequested' | 'Rejected'>('Draft');
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, COLLECTIONS.EVENTS, eventId)).then((snapshot) => {
      ...
      if (!['Draft', 'AmendmentRequested', 'Rejected'].includes(data.status)) throw new Error('This application can no longer be edited.');
      setForm({ ...(data.eventDetails as EventDetails), riskProfile: completeRiskProfile(data.eventDetails?.riskProfile) });
      setEditableStatus(data.status as 'Draft' | 'AmendmentRequested' | 'Rejected');
