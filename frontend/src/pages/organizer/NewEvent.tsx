...
import { applyM1ExtractedFields, bindCanonicalVenue, completeRiskProfile, createInitialEventDetails, createM1DraftRecord, extractionMatchesDraftDocuments, findUniqueRegistryVenueMatch, inferMalaysiaStateFromAddress, isEditableApplicationStatus, isMeaningfulNotApplicableReason, isSelectableRegistryVenue, isValidVenueLocation, MALAYSIA_STATES, nextVersionId, reconcileM1EvidenceManifest, unlinkedSupportingEvidence, validateEventApplication, validateTemplateCompatibility } from './organizerApplication';
...
export default function NewEvent() {
  ...
  const unlinkedSupportingUploads = unlinkedSupportingEvidence(draftDocuments, evidenceManifest);
  const unlinkedSupportingPaths = new Set(unlinkedSupportingUploads.map((document) => document.path));
  const evidenceNameCounts = new Map<string, number>();
  supportingUploads.forEach((document) => evidenceNameCounts.set(document.originalName, (evidenceNameCounts.get(document.originalName) ?? 0) + 1));
    ...
    <div>
        ...
        <nav aria-label="Step 5 application sections" className="border-x border-b border-brand-200 bg-[#fffdf8] text-sm">
          <div className="md:hidden">
            <button type="button" className="flex min-h-14 w-full items-center justify-between gap-3 px-4 text-left font-bold text-brand-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-500" aria-expanded={mobileSectionsOpen} aria-controls="mobile-application-sections" onClick={() => setMobileSectionsOpen(value => !value)}>
              ...
              <span className="flex items-center gap-3 text-xs text-ink-600"><span className={`rounded-full px-2 py-1 font-bold ${resolvedEvidenceCount === evidenceDefinitions.length && evidenceDefinitions.length && unlinkedSupportingUploads.length === 0 ? 'bg-brand-700 text-white' : 'bg-brand-100 text-brand-800'}`}>Evidence {resolvedEvidenceCount}/{evidenceDefinitions.length}{unlinkedSupportingUploads.length ? ` · ${unlinkedSupportingUploads.length} unlinked` : ''}</span><span className="flex items-center gap-1">{mobileSectionsOpen ? 'Collapse' : 'Expand'}<ChevronDown size={18} className={`transition-transform ${mobileSectionsOpen ? 'rotate-180' : ''}`} aria-hidden="true" /></span></span>
            </button>
      ...
      <form onSubmit={handleSubmit} noValidate className="rounded-lg border border-[#ded5c5] bg-[#fffdf8] shadow-card">
        ...
        <div className="space-y-8 p-4 sm:p-6 lg:p-8">
            ...
            {extraction && (
              <div className="sticky top-[148px] z-10 rounded-lg border border-brand-200 bg-brand-50 p-4 shadow-sm" role="status">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <span className="badge bg-white text-brand-700">{extraction.completionPercent}% of document fields read</span>
                </div>
                ...
                <p className="mt-3 text-xs text-ink-500">This percentage covers recognised fields in the two application documents. Venue map coordinates and supporting-evidence links must still be reviewed separately.</p>
              </div>
            )}
          </fieldset>
          ...
          <fieldset className="min-w-0 space-y-5">
            ...
            {!form.venueId && !GOOGLE_MAPS_API_KEY && form.venueLocation && !isValidVenueLocation(form.venueLocation) && <p role="alert" className="rounded-md border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">Saved venue coordinates are invalid. Enter a latitude from -90 to 90 and longitude from -180 to 180.</p>}
          ...
          <fieldset id="supporting-evidence" tabIndex={-1} style={{ scrollMarginTop: 155 }} className="min-w-0 space-y-4 border-t border-[#e3dacb] pt-8">
            ...
            <div className="rounded-lg border border-brand-200 bg-brand-50 p-4 shadow-sm">
              ...
              <span className={`badge mt-3 bg-white ${unlinkedSupportingUploads.length ? 'text-red-700' : 'text-brand-700'}`}>{resolvedEvidenceCount} / {evidenceDefinitions.length} checklist items complete · {completeEvidenceCount} evidence linked · {validNotApplicable.length} excluded with valid reason{incompleteNotApplicableCount ? ` · ${incompleteNotApplicableCount} reasons incomplete` : ''}{unlinkedSupportingUploads.length ? ` · ${unlinkedSupportingUploads.length} uploaded file${unlinkedSupportingUploads.length === 1 ? '' : 's'} unlinked` : ''}</span>
            ...
            <div className="rounded-lg border border-cream-200 p-4"><p className="text-sm text-ink-600">Upload supporting files together, then select the matching file for each requirement below. Check the document contents; filenames alone do not prove that evidence is correct.</p><label className="btn-secondary mt-3 cursor-pointer">Upload supporting files<input type="file" multiple accept=".pdf,.docx,.jpg,.jpeg,.png,.webp" disabled={uploading} onChange={event => handleFiles(event, 'supporting_evidence')} className="sr-only" /></label>{activeUpload?.role === 'supporting_evidence' && !activeUpload.requirementId && <UploadProgress upload={activeUpload} onCancel={cancelUpload} />}<ul className="mt-3 space-y-2">{supportingUploads.map((file, index) => { const duplicateIndex = supportingUploads.slice(0, index + 1).filter((candidate) => candidate.originalName === file.originalName).length; const duplicateCount = evidenceNameCounts.get(file.originalName) ?? 1; return <li key={file.path} className={`flex flex-wrap items-center justify-between gap-2 rounded-md border p-2 text-sm ${unlinkedSupportingPaths.has(file.path) ? 'border-red-200 bg-red-50' : 'border-transparent'}`}><span className="min-w-0 break-all">{file.originalName}{duplicateCount > 1 ? ` (${duplicateIndex} of ${duplicateCount})` : ''} {unlinkedSupportingPaths.has(file.path) ? <strong className="ml-2 whitespace-nowrap text-red-700">Not linked</strong> : <span className="ml-2 whitespace-nowrap text-brand-700">Linked</span>}</span><div><button type="button" className="btn-secondary" onClick={() => viewDocument(file.path)}>View</button><button type="button" className="btn-secondary ml-2" onClick={() => requestRemoveDocument(file.path)}>Remove</button></div></li>; })}</ul></div>
            <div className="space-y-4">
              {onlyMissing && completeEvidenceCount + validNotApplicable.length === evidenceDefinitions.length && <p role="status" className={`rounded-md p-4 text-sm ${unlinkedSupportingUploads.length ? 'border border-red-200 bg-red-50 text-red-700' : 'bg-brand-50'}`}>{unlinkedSupportingUploads.length ? `All checklist items are complete, but ${unlinkedSupportingUploads.length} uploaded file${unlinkedSupportingUploads.length === 1 ? ' is' : 's are'} not linked. Remove or link ${unlinkedSupportingUploads.length === 1 ? 'it' : 'them'} before submitting.` : 'All evidence items are complete. Turn off the filter to review them.'}</p>}
                ...
                return <article id={`evidence-${definition.id}`} tabIndex={-1} style={{ scrollMarginTop: 290 }} key={definition.id} className="rounded-lg border border-[#ded5c5] bg-cream-50 p-4">
                  ...
                  {response.applicability === 'not_applicable' && !forcedRequired ? <div className="mt-4"><label className="field-label" htmlFor={`reason-${definition.id}`}>Why this does not apply *</label><textarea id={`reason-${definition.id}`} className="input mt-1 min-h-20" minLength={20} maxLength={500} aria-describedby={`reason-help-${definition.id}`} value={response.notApplicableReason ?? ''} onChange={(event) => updateEvidenceResponse(definition.id, { requirementId: definition.id, applicability: 'not_applicable', notApplicableReason: event.target.value })} /><p id={`reason-help-${definition.id}`} className={`mt-1 text-xs ${response.notApplicableReason && !isMeaningfulNotApplicableReason(response.notApplicableReason) ? 'font-semibold text-red-700' : 'text-ink-500'}`}>Give a specific operational reason using at least 20 characters and 3 words. This declaration is retained for reviewer audit.</p></div> : <div className="mt-4 space-y-3">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      {supportingUploads.length > 0 && <select aria-label={`${definition.id} existing evidence`} className="input min-w-0 flex-1" value={response.documentPath ?? ''} onChange={(event) => updateEvidenceResponse(definition.id, { requirementId: definition.id, applicability: 'required', ...(event.target.value ? { documentPath: event.target.value } : {}) })}><option value="">Choose an uploaded file</option>{supportingUploads.map((document, index) => { const duplicateIndex = supportingUploads.slice(0, index + 1).filter((candidate) => candidate.originalName === document.originalName).length; const duplicateCount = evidenceNameCounts.get(document.originalName) ?? 1; return <option key={document.path} value={document.path}>{document.originalName}{duplicateCount > 1 ? ` (${duplicateIndex} of ${duplicateCount})` : ''}{unlinkedSupportingPaths.has(document.path) ? ' — not linked' : ''}</option>; })}</select>}
      ...
      <aside className="sticky top-24 hidden rounded-lg border border-brand-200 bg-[#fffdf8] p-3 shadow-card 2xl:block" aria-label="On this application page">
        ...
        <div className="mt-3 border-t border-brand-100 px-2 pt-3">
          <p className={`text-xs font-semibold ${unlinkedSupportingUploads.length ? 'text-red-700' : 'text-ink-600'}`}>Evidence {resolvedEvidenceCount}/{evidenceDefinitions.length}{unlinkedSupportingUploads.length ? ` · ${unlinkedSupportingUploads.length} unlinked` : ''}</p>
