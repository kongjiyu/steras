    ...
    <div className="min-h-screen bg-[#f3f1e9] pb-16">
      ...
      <main className="page-shell page-enter">
        ...
        <section className="overflow-x-auto rounded-lg border border-[#ded5c5] bg-white shadow-card">
          <header className="grid min-w-[850px] grid-cols-[1.5rem_minmax(0,2fr)_minmax(0,1.4fr)_9rem_7rem_7rem] gap-3 border-b border-[#e8e0cf] bg-cream-50 px-4 py-2.5 text-[11px] font-bold uppercase tracking-[0.06em] text-ink-500">
          ...
          {loading ? (
            ...
            <ul className="min-w-[850px] divide-y divide-[#e8e0cf]">
              {filtered.map((e) => {
                // Per-row "Assign" link is hidden when the event is
                // past the assignment stage (second review, closed).
                const showAssign = !e.assignedOfficerUids?.length && e.initialReview?.decision === 'Approved' && e.reviewStage !== 'second'
                  && !['Approved', 'Rejected', 'Withdrawn'].includes(e.status);
                return (
                  <li key={e.eventId} className="relative flex items-stretch">
                    <Link
                      to={`/admin/applications/${e.eventId}`}
                      className="admin-queue-row grid flex-1 grid-cols-[1.5rem_minmax(0,2fr)_minmax(0,1.4fr)_9rem_7rem_7rem] items-center gap-3 px-4 py-3 transition hover:bg-cream-50"
                    >
                    ...
                    {showAssign && (
                      <Link
                        ...
                        className="absolute right-10 top-1/2 flex -translate-y-1/2 items-center gap-1 rounded px-2 py-3 text-xs font-semibold text-brand-700 transition hover:bg-cream-50"
