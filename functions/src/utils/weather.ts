...
import { WeatherContext, VenueLocation } from '@shared/types';
...
export async function fetchWeather(
  ...
  if (cached && Date.now() - (cached as WeatherContext & { _ts: number })._ts < CACHE_TTL_MS) {
    return cached;
  }
  ...
  try {
    ...
    CACHE.set(cacheKey, { ...result, _ts: Date.now() } as WeatherContext);
    return result;
  } catch (err) {
    // eslint-disable-next-line no-console
