...
export async function computeRuleBased(
  ...
  incidents: Incident[],
  now = Date.now(),
  sourceTimestamps: Partial<Record<'weather' | 'history' | 'holiday', number>> = {},
  historyMatched = true,
): Promise<DeterministicBaseline> {
  const subScores = {
  ...
  return {
    ...
    baselineScore,
    baselineRiskLevel: riskLevelFor(baselineScore),
    evidence: [
...
function scoreWeather(weather: WeatherContext): number {
  let score = weather.severeAlert ? 50 : 0;
  const forecast = (weather.forecast ?? '').toLowerCase();
  if (forecast.includes('thunder')) score += 35;
  else if (forecast.includes('rain') || forecast.includes('shower')) score += 20;
  else if (forecast.includes('cloud') || forecast.includes('overcast')) score += 8;
  else score += 2;
  if (weather.precipitationProbability > 70) score += 15;
  if (weather.windSpeed > 10) score += 10;
  if (weather.temperature > 35 || weather.temperature < 18) score += 8;
  return clamp(Math.round(score), 0, 100);
}

function scoreCrowd(event: EventRecord): number {
  const { venueCapacity, expectedAttendance } = event.eventDetails;
  if (venueCapacity <= 0) return 50;
  ...
  if (expectedAttendance > 10_000) score += 30;
  else if (expectedAttendance > 5_000) score += 20;
  else if (expectedAttendance > 1_000) score += 10;
  else if (expectedAttendance > 250) score += 4;
  return clamp(Math.round(score), 0, 100);
}
...
function scoreVenue(event: EventRecord): number {
  ...
  let score = 15;
  if (venueCapacity > 20_000) score += 25;
  else if (venueCapacity > 5_000) score += 15;
  else if (venueCapacity > 1_000) score += 8;
  if (type === 'concert' || type === 'religious') score += 15;
  if (environment !== 'indoor') score += 10;
  if (coverage === 'uncovered') score += 10;
  else if (coverage === 'partially_covered') score += 5;
  return clamp(Math.round(score), 0, 100);
}
...
export async function fetchIncidentsForVenue(venueId?: string, venueName?: string, now = Date.now()): Promise<IncidentSnapshot> {
  ...
  if (!resolvedVenueId && venueName) {
    const venueSnapshot = await db.collection(COLLECTIONS.VENUES).where('name', '==', venueName.trim()).limit(1).get();
    resolvedVenueId = venueSnapshot.docs[0]?.id;
  }
  if (!resolvedVenueId) return { incidents: [], matched: false, fetchedAt: now };
  const snapshot = await db.collection(COLLECTIONS.INCIDENTS).where('venueId', '==', resolvedVenueId).get();
  return {
