...
import { decodeStage1Source } from '../utils/stage1Redaction';
...
export const publishStage1Doc = onCall<Stage1PublicationRequest>({ region: FUNCTION_REGION }, async (request) => {
  ...
  const isDeclaration = stage1Doc.usePreviousDeclaration === true && !stage1Doc.filePath && stage1Doc.status === 'use_previous';
  if (!isActiveControlGeneration(event, control, eventId) || (stage1Doc.status !== 'verified' && !isDeclaration)) throw new HttpsError('failed-precondition', 'Only a current Authority-verified Stage 1 document or Use Previous declaration can be published.');
  ...
  const decodedSource = draft && stage1Doc.filePath ? await decodeStage1Source(stage1Doc.filePath) : null;
  if (draft && stage1Doc.filePath) {
    if (!decodedSource || decodedSource.sha256 !== draft.sourceHash) throw new HttpsError('aborted', 'The source document changed. Generate a new redaction draft.');
  }
  const now = Date.now();
  ...
  const projection: PublicStage1Document = {
    ...
    ...(isDeclaration ? {} : { fileUrl: draft!.redactedFilePath, fileMimeType: decodedSource?.mimeType ?? stage1Doc.filePath?.slice(5, stage1Doc.filePath.indexOf(';')) ?? 'application/octet-stream' }),
  ...
  await db.runTransaction(async (tx) => {
    ...
    const freshDeclaration = isDeclaration && freshDocData.status === 'use_previous'
      && freshDocData.usePreviousDeclaration === true && !freshDocData.filePath;
    if (!isActiveControlGeneration(freshEventData, freshControlData, eventId) || freshDocData.revisionId !== revisionId
      || (freshDocData.status !== 'verified' && !freshDeclaration)) throw new HttpsError('aborted', 'The Stage 1 revision is no longer current.');
