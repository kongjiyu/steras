...
import { COLLECTIONS, EventRecord, RESOURCE_KEYS, ResourceQuantities, ResourceRecommendation, UserProfile } from '@shared/types';
...
export async function overrideResourcesForUser(uid: string, request: OverrideResourcesRequest, now = Date.now()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const updatedItems = Object.fromEntries(RESOURCE_KEYS.map((key) => [key, {
      ...previous.items[key],
    ...
    const updated = {
      ...previous,
      items: updatedItems,
      confidenceLevel: 'authority_validated' as const,
      authorityReviewRequired: false,
      notes: `Authority override: ${rationale}`,
      overriddenBy: uid,
      overrideRationale: rationale,
      overriddenAt: now,
    } as unknown as ResourceRecommendation & { overriddenBy: string; overrideRationale: string; overriddenAt: number };
...
export function throwResourceOverridesUnavailable(): never {
  throw new HttpsError('failed-precondition', 'Resource adjustments are unavailable until the append-only authority finalisation workflow is enabled.');
}
