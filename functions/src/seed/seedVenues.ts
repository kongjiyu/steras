...
import { COLLECTIONS, Venue, Incident, EventType } from '@shared/types';
...
function generateIncidents(venueId: string, venueName: string, count: number): Omit<Incident, 'incidentId'>[] {
  const incidents: Omit<Incident, 'incidentId'>[] = [];
  const now = Date.now();
  for (let i = 0; i < count; i++) {
    ...
    const sev = (['low', 'low', 'low', 'medium', 'medium', 'high'] as const)[Math.floor(Math.random() * 6)];
    incidents.push({
...
async function seed() {
  ...
  for (const v of VENUES) {
    const ref = db.collection(COLLECTIONS.VENUES).doc();
    const venue: Venue = { ...v, venueId: ref.id };
    await ref.set(venue);
    ...
    for (const inc of incidents) {
      await db.collection(COLLECTIONS.INCIDENTS).add(inc);
    }
    // eslint-disable-next-line no-console
