...
function validateDraftDocuments(eventId, versionId, value) {
    if (!Array.isArray(value))
        throw new https_1.HttpsError('failed-precondition', 'Upload the completed Core and scenario PDF/DOCX files before extraction.');
    const documents = value;
    if (documents.length < 1)
        throw new https_1.HttpsError('failed-precondition', 'Upload the two completed Core and scenario PDF/DOCX files.');
    if (documents.length > 20)
        throw new https_1.HttpsError('failed-precondition', 'The Draft document list exceeds the 20-file limit.');
    if (documents.some((document) => document?.role === 'combined_application')) {
        throw new https_1.HttpsError('failed-precondition', 'Combined application files are no longer accepted. Upload the completed Core and scenario files separately.');
    }
    ...
    if (coreCount !== 1 || scenarioCount !== 1) {
        throw new https_1.HttpsError('failed-precondition', 'Upload exactly one completed Core PDF/DOCX and one completed scenario PDF/DOCX.');
    }
    const paths = new Set();
    for (const document of documents) {
        ...
        if (document.role !== 'supporting_evidence' && !isApplicationDocumentFormat(document)) {
            throw new https_1.HttpsError('invalid-argument', 'Completed application documents must be PDF or DOCX files with matching file extensions.');
        }
        if (document.role === 'supporting_evidence' && !EVIDENCE_MIME_TYPES.has(document.mimeType)) {
            throw new https_1.HttpsError('invalid-argument', 'Supporting evidence metadata must identify a PDF, DOCX, JPEG, PNG, or WebP file.');
        }
    }
    return documents;
}
...
function isApplicationDocumentFormat(document) {
