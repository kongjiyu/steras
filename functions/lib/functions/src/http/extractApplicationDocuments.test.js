...
function document(role, name, mimeType = name.endsWith('.pdf') ? pdfMime : docxMime) {
    return {
...
(0, vitest_1.describe)('M1 structured Draft document validation', () => {
    (0, vitest_1.it)('accepts PDF or DOCX independently for the Core and scenario files', () => {
    ...
    (0, vitest_1.it)('rejects mismatched application file extensions', () => {
    ...
    (0, vitest_1.it)('rejects legacy combined application uploads with migration guidance', () => {
        const combined = document('core_template', 'combined.pdf');
        (0, vitest_1.expect)(() => (0, extractApplicationDocuments_1.validateDraftDocuments)('event-1', 'v1', [
            { ...combined, role: 'combined_application' },
    ...
    (0, vitest_1.it)('rejects missing, duplicate, swapped-format, cross-version, and duplicate-path documents', () => {
        ...
        (0, vitest_1.expect)(() => (0, extractApplicationDocuments_1.validateDraftDocuments)('event-1', 'v1', [core])).toThrow('exactly one completed Core PDF/DOCX');
        (0, vitest_1.expect)(() => (0, extractApplicationDocuments_1.validateDraftDocuments)('event-1', 'v1', [core, { ...core, originalName: 'copy.docx' }, scenario])).toThrow('exactly one completed Core PDF/DOCX');
        (0, vitest_1.expect)(() => (0, extractApplicationDocuments_1.validateDraftDocuments)('event-1', 'v1', [core, { ...scenario, mimeType: pdfMime }]))
            .toThrow('must be PDF or DOCX files with matching file extensions');
