...
const adminUserManagement_1 = require("../http/adminUserManagement");
const adminVenueManagement_1 = require("../http/adminVenueManagement");
...
const app = (0, app_1.initializeApp)({ projectId });
const auth = (0, auth_1.getAuth)(app);
const db = (0, firestore_1.getFirestore)(app);
const handlers = { createPrivilegedAccount: adminUserManagement_1.createPrivilegedAccount, saveVenue: adminVenueManagement_1.saveVenue, verifyVenue: adminVenueManagement_1.verifyVenue, deactivateVenue: adminVenueManagement_1.deactivateVenue };
async function main() {
    ...
    const createdAuth = await auth.getUser(account.uid);
...
async function resetUatArtifacts() {
    const oldAccount = await auth.getUserByEmail('pdrm.officer@steras.test').catch((error) => {
...
function accountInput(idempotencyKey) {
...
async function seedUser(uid, email, password, name, role) {
    await auth.createUser({ uid, email, password, displayName: name }).catch((error) => {
...
async function signIn(email, password) {
    ...
    if (!response.ok || !data.idToken || !data.localId)
        throw new Error(`Unable to sign in ${email}.`);
    return data.localId;
}
async function call(name, uid, data) {
    return handlers[name].run({ auth: { uid }, data });
}
async function expectError(run, httpStatus, functionStatus) {
    try {
        await run();
    }
    catch (error) {
...
//# sourceMappingURL=runM1AdminVenueUat.js.map