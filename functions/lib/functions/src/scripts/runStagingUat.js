...
const requestedScenarios = (process.env.UAT_SCENARIOS ?? 'assessment,manual,outside_horizon,insufficient,withdrawal')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);
const allowedScenarios = new Set(['assessment', 'manual', 'outside_horizon', 'insufficient', 'withdrawal']);
...
if (requestedScenarios.length === 0 || requestedScenarios.some((scenario) => !allowedScenarios.has(scenario))) {
    throw new Error('UAT_SCENARIOS contains an unsupported scenario.');
}
...
const authorityTypes = ['PDRM', 'BOMBA', 'KKM'];
async function run() {
    ...
    for (const authorityType of authorityTypes) {
        authorities.set(authorityType, await provisionUser(`uat-${authorityType.toLowerCase()}@steras.test`, `${authorityType} UAT Reviewer`, 'authority', authorityType));
    }
    const results = [];
    for (const [index, scenario] of requestedScenarios.entries()) {
        ...
        if (scenario === 'assessment')
            results.push(await runAssessment(context));
        if (scenario === 'manual')
            results.push(await runManualAssessment(context));
        if (scenario === 'outside_horizon')
            results.push(await runOutsideHorizon(context));
        if (scenario === 'insufficient')
            results.push(await runInsufficientEvidence(context));
        if (scenario === 'withdrawal')
            results.push(await runWithdrawal(context));
    }
...
async function createScenario(scenario, index, organizer, authorities, admin) {
    const runId = `${Date.now()}-${(0, node_crypto_1.randomUUID)().slice(0, 8)}`;
    const eventId = `uat-${scenario}-${runId}`;
    const venueId = `uat-venue-${runId}`;
    const startDatetime = Date.now() + (scenario === 'outside_horizon' || scenario === 'manual' ? 30 + index : 3 + index) * 24 * 60 * 60 * 1_000;
    const eventDetails = {
        ...
        riskProfile: {
            ...
            authorityCoordinationConfirmed: true, vulnerableAttendeesPercent: 5, standingAttendeesPercent: 0,
...
async function runAssessment(context) {
    ...
    if (!event.requiredAuthorities.includes('PDRM'))
        throw new Error(`PDRM was not assigned to ${context.eventId}.`);
    await assertProvisionalDecisionBlocked(context, 'PDRM');
    if (!('provisionalResult' in assessment) || assessment.aiProposal.status !== 'success')
        throw new Error('A successful provisional assessment is required.');
    ...
    if (official.status !== 'official_ready')
        throw new Error(`Expected official_ready after all authority reviews, received ${official.status}.`);
    const currentEvent = await eventFor(context.eventId);
    if (!currentEvent.currentResourceId)
        throw new Error('Official finalisation did not publish a current resource revision.');
    const resource = await db.doc(`${types_1.COLLECTIONS.EVENTS}/${context.eventId}/${types_1.COLLECTIONS.RESOURCES}/${currentEvent.currentResourceId}`).get();
    if (resource.data()?.stage !== 'official')
        throw new Error('The current resource revision is not official.');
    return verifyScenario(context.eventId, 'assessment', 'Pending', 1, false, official);
}
async function runWithdrawal(context) {
...
async function runManualAssessment(context) {
    const { assessment } = await submitAndAssess(context, 'v1', true);
    if (assessment.status !== 'manual_review_required') {
        const { provisionalResult: _result, authorityReviewState: _state, officialResult: _official, ...base } = assessment;
        void _result;
        void _state;
        void _official;
        const current = await eventFor(context.eventId);
        if (!current.currentAssessmentId)
            throw new Error('Manual UAT has no current assessment pointer.');
        await db.doc(`${types_1.COLLECTIONS.EVENTS}/${context.eventId}/${types_1.COLLECTIONS.ASSESSMENTS}/${current.currentAssessmentId}`).set({
            ...base, status: 'manual_review_required', authorityReviewRequired: true,
            aiProposal: { status: 'invalid', model: 'MiniMax-M2', promptVersion: 'uat', responseSchemaVersion: 'uat', retryable: true, errorSummary: 'UAT injected invalid response verifies Admin recovery without fabricated scores.', cacheStatus: 'not-applicable', generatedAt: Date.now() },
            manualReviewReason: 'UAT injected invalid AI response.',
        });
        await db.doc(`${types_1.COLLECTIONS.EVENTS}/${context.eventId}`).update({ currentResourceId: firestore_1.FieldValue.delete() });
    }
    ...
    if (!evidenceKey)
        throw new Error('Manual UAT requires at least one eligible evidence reference.');
    ...
    if (official.status !== 'official_ready' || !('sourceKind' in official) || official.sourceKind !== 'admin_manual')
        throw new Error('Manual UAT did not publish admin_manual official output.');
    const event = await eventFor(context.eventId);
    const resource = event.currentResourceId ? await db.doc(`${types_1.COLLECTIONS.EVENTS}/${context.eventId}/${types_1.COLLECTIONS.RESOURCES}/${event.currentResourceId}`).get() : undefined;
    if (resource?.data()?.assessmentReference?.sourceKind !== 'admin_manual')
        throw new Error('Manual UAT resource provenance is not proposal-free.');
    return verifyScenario(context.eventId, 'manual', 'Pending', 1, false, official);
}
async function runOutsideHorizon(context) {
    const { assessment } = await submitAndAssess(context, 'v1');
    if (assessment.status !== 'provisional_ready'
        || assessment.contextSnapshot.weather.data !== null
        || assessment.contextSnapshot.weather.unavailableReason !== 'outside_forecast_horizon') {
        throw new Error('Outside-horizon UAT did not preserve explicit unavailable weather without placeholder measurements.');
    }
    return verifyScenario(context.eventId, 'outside_horizon', 'Pending', 1, false, assessment);
}
async function runInsufficientEvidence(context) {
    const { assessment } = await submitAndAssess(context, 'v1', true);
    if (assessment.status !== 'manual_review_required' || assessment.assessmentReadiness !== 'insufficient_data'
        || assessment.aiProposal !== null) {
        throw new Error('Custom-venue UAT did not short-circuit to manual review before MiniMax.');
    }
    const event = await eventFor(context.eventId);
    if (event.currentResourceId)
        throw new Error('Insufficient-evidence UAT published a resource unexpectedly.');
    return verifyScenario(context.eventId, 'insufficient', 'Pending', 1, false);
}
async function submitAndAssess(context, expectedVersionId, allowManual = false) {
    const token = await idTokenFor(context.organizer.profile.email);
    const submission = await callFunction('submitEvent', token, { eventId: context.eventId });
    if (submission.versionId !== expectedVersionId)
        throw new Error(`Expected ${expectedVersionId}, received ${submission.versionId}.`);
    ...
    if (!('provisionalResult' in assessment)
        ...
        if (allowManual && assessment.status === 'manual_review_required')
...
async function assertProvisionalDecisionBlocked(context, authorityType) {
    const reviewer = requiredAuthority(context.authorities, authorityType);
    try {
        ...
        if (error instanceof Error && /official risk assessment/i.test(error.message))
            return;
        throw error;
    }
    throw new Error(`Provisional assessment unexpectedly allowed a final decision for ${context.eventId}.`);
}
async function verifyScenario(eventId, scenario, expectedStatus, expectedVersions, expectedPublic, assessment) {
    const event = await eventFor(eventId);
    const versions = await db.collection(`${types_1.COLLECTIONS.EVENTS}/${eventId}/${types_1.COLLECTIONS.VERSIONS}`).get();
    const publicRecord = await db.collection(types_1.COLLECTIONS.PUBLIC_EVENTS).doc(eventId).get();
...
async function provisionUser(email, name, role, authorityType) {
    ...
    const now = Date.now();
    const current = await db.collection(types_1.COLLECTIONS.USERS).doc(user.uid).get();
    const profile = {
        uid: user.uid,
        name,
        email,
...
async function idTokenFor(email) {
    ...
    if (!response.ok || !body.idToken)
        throw new Error(`Token exchange failed: ${body.error?.message ?? response.status}`);
    return body.idToken;
}
async function callFunction(name, token, data) {
    ...
    const body = await response.json();
    if (!response.ok)
        throw new Error(`${name} failed: ${body.error?.message ?? response.status}`);
    const result = body.result ?? body.data;
    if (!result)
        throw new Error(`${name} returned no result.`);
    return result;
}
async function waitForAssessment(eventId, versionId, allowManual = false) {
    ...
    while (Date.now() < deadline) {
        ...
        if (event?.currentAssessmentId && allowManual && await assessmentIsManualReview(eventId, event.currentAssessmentId))
            return event;
        if (event?.currentAssessmentId && event.currentResourceId) {
            const resource = await eventReference.collection(types_1.COLLECTIONS.RESOURCES).doc(event.currentResourceId).get();
            if (resource.exists
                && resource.data()?.schemaVersion === types_1.RESOURCE_SCHEMA_VERSION
                && resource.data()?.stage === 'provisional'
                && resource.data()?.versionId === versionId)
                return event;
        }
        const assessment = event?.currentAssessmentId
            ? await eventReference.collection(types_1.COLLECTIONS.ASSESSMENTS).doc(event.currentAssessmentId).get()
            : undefined;
        if (assessment?.data()?.status === 'failed')
            throw new Error(`Assessment failed: ${assessment.data()?.error ?? 'unknown error'}`);
        await new Promise((resolve) => setTimeout(resolve, 2_000));
    }
    throw new Error(`Assessment ${eventId}/${versionId} did not finish within 120 seconds.`);
}
async function eventFor(eventId) {
    const snapshot = await db.collection(types_1.COLLECTIONS.EVENTS).doc(eventId).get();
    if (!snapshot.exists)
        throw new Error(`Event ${eventId} does not exist.`);
    return snapshot.data();
}
async function assessmentFor(eventId, versionId) {
    const event = await eventFor(eventId);
    if (event.currentVersionId !== versionId || !event.currentAssessmentId)
        throw new Error(`Assessment ${eventId}/${versionId} is not current.`);
    const snapshot = await db.doc(`${types_1.COLLECTIONS.EVENTS}/${eventId}/${types_1.COLLECTIONS.ASSESSMENTS}/${event.currentAssessmentId}`).get();
...
async function assessmentIsManualReview(eventId, assessmentId) {
    const snapshot = await db.doc(`${types_1.COLLECTIONS.EVENTS}/${eventId}/${types_1.COLLECTIONS.ASSESSMENTS}/${assessmentId}`).get();
    return snapshot.data()?.status === 'manual_review_required';
}
function requiredAuthority(authorities, authorityType) {
    const authority = authorities.get(authorityType);
    if (!authority)
        throw new Error(`${authorityType} UAT account was not provisioned.`);
    return authority;
}
function assessmentSummary(assessment) {
    ...
    if (!('provisionalResult' in assessment))
        throw new Error('Assessment requires manual review and has no calculated result.');
    return {
...
//# sourceMappingURL=runStagingUat.js.map