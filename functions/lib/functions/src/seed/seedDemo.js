...
const seedVenues_1 = require("./seedVenues");
...
const app = (0, app_1.initializeApp)({ credential: (0, app_1.applicationDefault)(), projectId: PROJECT_ID, storageBucket: `${PROJECT_ID}.appspot.com` }, 'steras-demo-seed');
...
async function seedDemo() {
    ...
    if (!organizer)
        throw new Error('Demo organizer was not provisioned.');
    ...
    for (const venue of venues) {
        batch.set(db.collection(types_1.COLLECTIONS.VENUES).doc(venue.venueId), venue);
    }
    for (const historical of historicalEvents) {
        batch.set(db.collection(types_1.COLLECTIONS.HISTORICAL_EVENTS).doc(historical.historicalEventId), historical);
    }
    for (const incident of incidents) {
        batch.set(db.collection(types_1.COLLECTIONS.INCIDENTS).doc(incident.incidentId), incident);
    }
    for (const application of applications) {
        const eventReference = db.collection(types_1.COLLECTIONS.EVENTS).doc(application.event.eventId);
        batch.set(eventReference, application.event);
        batch.set(eventReference.collection(types_1.COLLECTIONS.VERSIONS).doc(application.version.versionId), application.version);
        batch.set(eventReference.collection(types_1.COLLECTIONS.ASSESSMENTS).doc(application.assessment.assessmentId), application.assessment);
    }
    batch.set(db.collection(types_1.COLLECTIONS.DATASET_MANIFESTS).doc(DATASET_VERSION), {
...
async function provisionAccounts() {
    ...
    for (const account of accounts) {
        ...
        await db.collection(types_1.COLLECTIONS.USERS).doc(user.uid).set(profile, { merge: true });
        profiles.push(profile);
    }
    return profiles;
}
function buildVenues() {
    return seedVenues_1.VENUES.map((fixture, index) => {
        const venueId = (0, seedVenues_1.stableVenueId)(fixture.name);
...
function buildHistoricalEvents(venues) {
    const eventTypes = ['concert', 'festival', 'sports', 'cultural', 'religious', 'exhibition', 'fair', 'conference'];
...
function buildIncidents(historicalEvents) {
    return Array.from({ length: 50 }, (_, index) => {
        const historical = historicalEvents[Math.floor(index / 2) * 8 + (index % 2)];
        const severity = index < 32 ? 'low' : index < 46 ? 'medium' : 'high';
        const status = index < 40 ? 'verified' : index < 44 ? 'under_review' : 'rejected';
        return {
...
function buildApplications(venues, organizerId) {
    const types = ['concert', 'festival', 'sports', 'cultural', 'religious', 'exhibition'];
    return Array.from({ length: 12 }, (_, index) => {
        ...
        const official = (0, ruleBased_1.computeCategoryBasedAssessment)(event, context, NOW);
        const assessment = {
            assessmentId,
            eventId,
            versionId,
...
function demoContext(event, venue, index) {
    return {
        weather: {
            data: null,
