...
const RISK_FLAG_BY_REQUIREMENT = {
...
function m1EvidenceRequirementsFor(scenarioTemplateId) {
    const count = SCENARIO_COUNTS[scenarioTemplateId];
    const prefix = scenarioTemplateId.match(/STERAS-(T\d{2})-/)?.[1];
    if (!count || !prefix)
        return [];
    return [
        ...exports.M1_CORE_EVIDENCE_REQUIREMENT_IDS.map((id) => ({ id, source: 'core', requirement: 'always' })),
        ...Array.from({ length: count }, (_, index) => {
            const id = `${prefix}-DOC-${String(index + 1).padStart(2, '0')}`;
            return {
                id,
                source: 'scenario',
                requirement: ALWAYS_SCENARIO_IDS.has(id) ? 'always' : 'conditional',
